...
async function trackBuilds() {
    try {
        ...
        vscode.window.showErrorMessage(`Failed to track builds: ${error instanceof Error ? error.message : String(error)}`, 'Open Folder').then(selection => {
...
async function runBuildProcess() {
    try {
        ...
        // Start build process
        const buildProcess = (0, child_process_1.spawn)(shell, [...shellArgs, finalCommand], {
            cwd: workspacePath,
            env: process.env,
...
function getValidatedBuildCommand() {
    let buildCommand = vscode.workspace.getConfiguration('build-logger').get('buildCommand') || '';
    const isWindows = process.platform === 'win32';
...
function showBuildDashboard() {
    try {
        const panel = vscode.window.createWebviewPanel('buildLoggerDashboard', 'Build Failure Dashboard', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage(async (message) => {
            try {
                ...
                vscode.window.showErrorMessage(`Error handling dashboard command: ${error instanceof Error ? error.message : String(error)}`);
            }
        }, undefined, []);
    }
    catch (error) {
        vscode.window.showErrorMessage(`Failed to create build dashboard: ${error instanceof Error ? error.message : String(error)}`);
    }
}
async function loadAndDisplayLogs(panel) {
    try {
        let logs = [];
        if (fs.existsSync(logFilePath)) {
            const fileData = await fs.promises.readFile(logFilePath, 'utf8');
        ...
        panel.webview.html = generateErrorHtml('Failed to load logs', error instanceof Error ? error.message : String(error));
    }
}
function generateDashboardHtml(logs) {
